  ...
  if (is_moving_ == false)
  {
    ...
    if (id_cnt != 0)
    {
      ...
      while(cnt < msg->points.size())
      {
	    ...
	    jnt_tra_point_msg.effort.clear();
          }
...
int main(int argc, char **argv)
{
  ...
  ros::Timer write_timer = node_handle.createTimer(ros::Duration(dynamixel_controller.getWritePeriod()/6), &DynamixelController::writeCallback, &dynamixel_controller);
